...
export default {
  ...
  mounted() {
    ...
    axios.get('http://localhost:8091/videos/view?id='+this.id).then((response) => {
      console.log(response)
    }).catch((error) =>{
      console.log(error)
    })

    axios.get('http://localhost:8091/videos/unique?id='+this.id).then((response) => {
      that.video = response.data
      that.commentsNumber = response.data.comments.length
      that.likes = response.data.likes
      that.link = 'http://localhost:8091/videos/videoFile?videoName='+response.data.videoLink
    }).catch((error) =>{
      console.log(error)
    })
    ...
    if(this.$store.state.user.id !== -1){
      axios.get('http://localhost:8091/videos/isLiked?id='+this.id+'&userId='+this.$store.state.userInfos.id).then((response) => {
  ...
  methods: {
    saveVideo(){
      axios.get('http://localhost:8091/videos/saveVideo?videoId='+this.id+'&userId='+this.$store.state.userInfos.id).then((response) => {
        console.log(response)
    ...
    updateLike(){
      axios.get('http://localhost:8091/videos/isLiked?id='+this.id+'&userId='+this.$store.state.userInfos.id).then((response) => {
        if(!response.data){
          axios.get('http://localhost:8091/videos/like?id='+this.id+'&userId='+this.$store.state.userInfos.id).then((response) => {
            console.log(response)
          }).catch((error) =>{
            console.log(error)
          })
